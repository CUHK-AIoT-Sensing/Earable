...
class Embed_Attention(nn.Module):
    def __init__(self, num_embeddings, C, F, num_heads=8):
        super(Embed_Attention, self).__init__()
        ...
        self.attention = nn.MultiheadAttention(embed_dim=C * num_embeddings * F, num_heads=num_heads)

    def forward(self, embeddings):
        # Assume embeddings is a list of tensors with shape [B, C, T, F]
        # Concatenate along the channel dimension
        concatenated_embeddings = torch.cat(embeddings, dim=1)  # Shape: [B, C*num_embeddings, T, F]

        # Reshape for attention: [B, C*num_embeddings, T, F] -> [B, T, C*num_embeddings*F]
        B, _, T, F = concatenated_embeddings.shape
        concatenated_embeddings = concatenated_embeddings.view(B, T, -1)  # Shape: [B, T, C*num_embeddings*F]
        ...
        # Reshape back to [B, T, C, F]
        output = attn_output.view(B, T, self.num_embeddings, self.C, self.F)  # Shape: [B, T, num_embeddings, C, F]

        # Optionally, reduce across the time dimension T
        output = output.mean(dim=1)  # Shape: [B, num_embeddings, C, F]

        return output
        
class Embed_to_Feats(nn.Module):
    '''
    Support the two types of embeddings, actually they are the same:
    1) [B, 1, Embeddings]
    2) [B, T, Embeddings]
    '''
    def __init__(self, spk_emb_dim, emb_dim, n_freqs):
        super(Embed_to_Feats, self).__init__()
    ...
    def forward(self, x):
        n_batch = x.shape[0]
        T = x.shape[1]
        x = x.reshape(n_batch * T, self.spk_emb_dim) # [B*T, Embeddings]
        x = self.proj(x) # [B*T, C * F]
        x = x.reshape([n_batch, T,  self.emb_dim, self.n_freqs]).permute(0, 2, 1, 3) # [B, C, T, F]
        return x
