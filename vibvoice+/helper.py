...
def superresolution_train(model, sample, optimizer, device='cuda', discriminator=None, optimizer_disc=None):
    ...
    est_mag = model(noisy_mag, acc, dvector)
    batch, channel, time, freq = est_mag.shape
    est_audio = istft((est_mag.reshape(-1, time, freq), noisy_phase.reshape(-1, time, freq)), 640, 320, 640, input_type="mag_phase")
    loss = sisnr(est_audio, clean.reshape(-1, time, freq))
    loss.backward() 
    optimizer.step()
    return loss.item()
